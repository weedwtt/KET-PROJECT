...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // ตรวจสอบสิทธิ์: role โดยตรง หรือ ได้รับมอบอำนาจ
  ...
  if (!teacher) {
    const delegateRecord = await db.approvalDelegate.findFirst({
      where: { delegateId: Number(teacherId) },
    })
    if (!delegateRecord) {
      return Response.json({ error: "ไม่พบครูที่มีสิทธิ์อนุมัติ" }, { status: 403 })
    }
  }
  ...
  const record = await db.statementRecord.update({
    ...
    data: {
      ...
      signatureTeacherId: signatureTeacherId ? Number(signatureTeacherId) : Number(teacherId),
      approvedAt: new Date(),
    },
  })
